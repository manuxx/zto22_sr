...
namespace Training.Specificaton {
    ...
    class when_trying_to_change_returned_collection_of_pets : pet_shop_concern {
        Establish c = () =>
            pet_initial_content.AddManyItems(new Pet { name = "Pixie" },
                new Pet { name = "Dixie" });
        ...
        It invalid_cast_exception_should_be_thrown = () =>
            exception.ShouldBeOfExactType<InvalidCastException>();
    }

    public class
        concern_with_pets_for_sorting_and_filtering : pet_shop_concern {
        private Establish c = () => {
            mouse_Dixie = new Pet {
                name = "Dixie",
    ...
    public class
        when_searching_for_pets : concern_with_pets_for_sorting_and_filtering {
        private It should_be_able_to_find_all_cats = () => {
            var criteria = Where<Pet>.HasAn(p => p.species)
        ...
        private It should_be_able_to_find_all_cats_or_dogs = () => {
            ...
            foundPets.ShouldContainOnly(cat_Tom, cat_Jinx, dog_Huckelberry,
                dog_Lassie, dog_Pluto);
        };
        ...
        private It should_be_able_to_find_all_pets_but_not_mice = () => {
            ...
            var foundPets = subject.AllPets().ThatSatisfy(criteria);
            foundPets.ShouldContainOnly(cat_Tom, cat_Jinx, dog_Huckelberry,
                dog_Lassie, dog_Pluto, rabbit_Fluffy);
        };

        private It should_be_able_to_find_all_pets_born_after_2010 = () => {
            var criteria = Where<Pet>
                .HasAn(p => p.yearOfBirth)
                .GreaterThan(2010);
            var foundPets = subject.AllPets().ThatSatisfy(criteria);
            foundPets.ShouldContainOnly(dog_Pluto, rabbit_Fluffy, mouse_Dixie,
                mouse_Jerry);
        };
    ...
    class when_sorting_pets : concern_with_pets_for_sorting_and_filtering {
        It should_be_able_to_sort_by_name_ascending = () => {
            ...
            result.ShouldContainOnlyInOrder(mouse_Dixie, rabbit_Fluffy,
                dog_Huckelberry, mouse_Jerry, cat_Jinx,
                dog_Lassie,
                dog_Pluto, cat_Tom);
        };
    };
